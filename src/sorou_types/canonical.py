...
def canonical_type(t: Type) -> Type:
    ...
    if isinstance(t, SumType):
        return SumType(tuple(sorted((canonical_type(part) for part in t.parts), key=format_type)))
    if isinstance(t, CompositeType):
        return CompositeType(
            top_prime=t.top_prime,
            f0=canonical_sorou(t.f0),
            subsidiary_types=tuple(
                sorted((canonical_type(part) for part in t.subsidiary_types), key=format_type)
            ),
        )
